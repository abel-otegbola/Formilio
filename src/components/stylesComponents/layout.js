...
export default function Layout({ styles, open, setOpen, handleTypo }) {

    return(
        <div className="p-2">
            <h3 className="flex items-center py-3 cursor-pointer" onClick={() => setOpen(1)}>{ open === 1 ? <FaCaretDown className="mr-2" /> : <FaCaretRight className="mr-2" />} Layout</h3>

            <div className={`${open === 1 ? "h-auto" : "h-0"} overflow-hidden transition-all duration-700`}>
