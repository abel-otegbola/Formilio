...
import { FiArrowRight, FiBell, FiSearch, FiUser } from "react-icons/fi";
    ...
    return (
        <div className="fixed w-full flex shadow-sm font-body justify-between items-center p-2 md:py-2 py-4 md:px-[10%] px-[5%] bg-white dark:bg-gray-900 border border-slate-50/[0.1] border-b-gray-100 dark:border-b-gray-200/[0.1] z-[100]">
            ...
            <ul className={`bg-white dark:bg-gray-900 flex md:flex-row flex-col md:w-auto h-full md:relative fixed top-0 right-0 transition-all duration-700 overflow-hidden ml-[4%] z-10 ${open ? "w-[75%]": "w-0"}`}>
                {(session) ? <a href="/dashboard" className="md:hidden flex justify-between p-4"><p>{session.user.email}</p> <FiArrowRight /></a> : ""}
